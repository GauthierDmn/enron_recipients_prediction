...
def predict(training_info_preprocessed, test_info_preprocessed):
    ...
    for sender, ids in emails_ids_per_sender.items():
        recs_temp = []
        for my_id in ids:
            recipients = training_info_preprocessed[training_info_preprocessed['mid'] == int(my_id)]['recipients'].tolist()
    ...
    for k, v in address_books.items():
        ...
        for (mail, val) in v: #
            temp_dictionary[mail] = temp_dictionary.get(mail, 0) + val
        address_books[k] = [(key, val) for (key, val) in temp_dictionary.items()]
    ...
    for key, value in emails_ids_per_sender_test.items():
        ...
        for id_predict in ids_predict:
            tfidf_test = tfidf_vector_test[test_info_preprocessed[test_info_preprocessed['mid'] == int(id_predict)].index]
            temp_list = []
